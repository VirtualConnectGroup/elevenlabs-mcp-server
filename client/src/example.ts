...
if (!process.env.MCP_SERVER_DIR) {
  throw new Error("MCP_SERVER_DIR environment variable is required");
}
...
async function main() {
  ...
  const client = new ElevenLabsClient("uv", [
    "--directory",
    process.env.MCP_SERVER_DIR as string,
    "run",
    "elevenlabs-mcp",
  ]);
  ...
  try {
    ...
    if (simpleResult.success && simpleResult.audioData) {
      ...
      writeFileSync(
        outputPath,
        Buffer.from(simpleResult.audioData.data, "base64")
      );
    ...
    const scriptResult = await client.generateScriptAudio({
      script: [
    ...
    if (scriptResult.success && scriptResult.audioData) {
      ...
      writeFileSync(
        outputPath,
        Buffer.from(scriptResult.audioData.data, "base64")
      );
      console.log("Audio saved to:", outputPath);
